...
class custom_env(gym.Env) :
  ...
  def __init__(self,prior_mean,prior_var,threshold_var,observation_noise=1,slot_time=1,initial_condition=0,Voc=3.63,Rs=0.1,Rp=0.03,Cp=500,Rl=0):
    ...
    self.action_space = self.action_space = spaces.Discrete(3) #spaces.MultiDiscrete([7,5])  #current action ranges from 0 to 6 and frequency action ranges from 0 to 5
  ...
  def reset_initial_condition(self): #reset_initial_conditions after every env.reset()
      ...
      #for Particle Filter
  def pfRs(self,prior_var,number_particles,prior_Voc,prior_Rs,prior_Rp,prior_tau,prior_Vc,observation_noise):
      self.test = ParticleFilterRs(prior_var=prior_var,number_particles=number_particles,prior_Voc=prior_Voc,prior_Rs=prior_Rs,prior_Rp=prior_Rp,prior_tau=prior_tau,prior_Vc=prior_Vc,observation_noise=observation_noise)
      
  def pfRp(self,prior_var,number_particles,prior_Voc,prior_Rs,prior_Rp,prior_tau,prior_Vc,observation_noise):
      self.test = ParticleFilterRp(prior_var=prior_var,number_particles=number_particles,prior_Voc=prior_Voc,Rs=prior_Rs,prior_Rp=prior_Rp,prior_tau=prior_tau,prior_Vc=prior_Vc,observation_noise=observation_noise)
      
  def pfVoc(self,prior_var,number_particles,prior_Voc,prior_Rs,prior_Rp,prior_tau,prior_Vc,observation_noise):
      self.test = ParticleFilterVoc(prior_var=prior_var,number_particles=number_particles,prior_Voc=prior_Voc,Rs=prior_Rs,Rp=prior_Rp,tau=prior_tau,prior_Vc=prior_Vc,observation_noise=observation_noise)
...
# =============================================================================
  ...
  def reset(self):
    ...
    """
      ...
      self.test.reset_prior(self.prior_var,self.prior_mean)
      self.ob=np.ones(30)
...
# =============================================================================
  ...
  def step(self,action):
    ...
    '''
    mean,cov,var=self.test.simulate(self.obs,amplitude=self.dk,frequency=self.freq,slot_time=self.slot_time) 
    ...
    if done:
       # self.initial_condition=0.0
        ...
        reward=-(1+self.dk**2) #70% weightage given to each iteration as cost and 30% to energy cost
